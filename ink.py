...
pv_access_key= os.getenv('PICO_KEY')
''''''
...
def render_time():
    Himage = Image.new('RGB', (display.width, display.height), 0xffffff)  # 255: clear the frame
    draw = ImageDraw.Draw(Himage)
    time_now = datetime.datetime.now()
    formatted_time = time_now.strftime("%d-%m-%Y %I:%M:%S %p")
    draw.text((10, 160), formatted_time.split()[0], font = fontBlack, fill = display.BLACK)
    draw.text((10, 200), formatted_time.split()[1], font = fontBlack, fill = display.ORANGE)
    draw.text((10, 240), formatted_time.split()[2], font = fontBlack, fill = display.GREEN)
    draw.text((10, 280), formatted_time.split()[3], font = fontBlack, fill = display.BLUE)
    draw.text((10, 320), formatted_time.split()[4], font = fontBlack, fill = display.RED)
    draw.text((10, 360), formatted_time.split()[5], font = fontBlack, fill = display.YELLOW)
    ...
    while True:
        ...
        if cobra.process(cobra_pcm) > 0.2:
            ...
            if silence_duration > 1.3:
                ...
                Himage = Image.new('RGB', (display.width, display.height), 0xffffff)  # 255: clear the frame
                draw = ImageDraw.Draw(Himage)
                draw.text((10, 160), "Couldn't hear you :(", font = fontBlack, fill = display.BLACK)
                draw.line((20, 50, 70, 100), fill = 0)
                draw.line((70, 50, 20, 100), fill = 0)
                draw.rectangle((20, 50, 70, 100), outline = 0)
                draw.line((165, 50, 165, 100), fill = 0)
...
def fade_leds(event):
    ...
    Himage = Image.new('RGB', (display.width, display.height), 0xffffff)  # 255: clear the frame
    draw = ImageDraw.Draw(Himage)
    draw.text((10, 160), "I'm Listening...", font = fontBlack, fill = display.BLACK)
    draw.line((20, 50, 70, 100), fill = 0)
    draw.line((70, 50, 20, 100), fill = 0)
    draw.rectangle((20, 50, 70, 100), outline = 0)
    draw.line((165, 50, 165, 100), fill = 0)
...
def wake_word():
    ...
    while Detect:
        ...
        if porcupine_keyword_index >= 0:
            Himage = Image.new('RGB', (display.width, display.height), 0xffffff)  # 255: clear the frame
            draw = ImageDraw.Draw(Himage)
            draw.text((10, 160), "Heard you!", font = fontBlack, fill = display.BLACK)
            draw.line((20, 50, 70, 100), fill = 0)
            draw.line((70, 50, 20, 100), fill = 0)
            draw.rectangle((20, 50, 70, 100), outline = 0)
            draw.line((165, 50, 165, 100), fill = 0)
